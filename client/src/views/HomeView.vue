...
export default {
  ...
  setup(props) {
		...
		const filteredFormations = computed(() => {
			return formations.value.filter((formation) => {
				if (
					...
					return (
						formation.hasOwnProperty("formationName") && 
						formation.formationName.toLowerCase().includes(filterByText.value) && 
						formation.hasOwnProperty("typeFormation") && 
						formation.typeFormation.toLowerCase().includes(inputFormationType.value) &&
    ...
    return { filteredFormations, filterByText, inputFormationType, inputStructure, inputAccess, inputBorneInf, inputBorneSup }
  }
}
</script>
	...
	<main>
		<form  class="filters">
			...
			<div>
				...
				<select name="typeFormation" id="typeFormation" v-model="inputFormationType">
					<option value="continue">Formation continue</option>
					<option value="initiale">Formation initiale</option>
					<option value="formation initiale et formation continue">Formation initiale et formation continue</option>
				...
				<select name="Structure" id="Structure" v-model="inputStructure">
					...
					<option value="public">Publi</option>
				...
				<select name="access" id="access" v-model="inputAccess">
					...
					<option value="distanciel et distanciel">Présentiel ou distanciel</option>
					<option value="">Vide</option>
				</select>

				<label for="minDuration">Entrez une durée min</label>
				<input type="number" name="minDuration" id="minDuration" min="0" v-model="inputBorneInf" @change="() => { if(inputBorneInf > inputBorneSup) {inputBorneSup = inputBorneInf + 1 }}">

				<label for="maxDuration">Entrez une durée max</label>
				<input type="number" name="maxDuration" id="maxDuration" min="0" v-model="inputBorneSup"  @change="() => { if(inputBorneSup <= inputBorneInf) { inputBorneSup = inputBorneInf + 1 }}">
			</div>
		</form>
