  ...
  <main>
    <HeaderInformation />
		...
		<div class="w-11/12 sm:w-4/5 md:w-3/4 lg:w-1/2 mx-auto">
			<form class="filters flex-col justify-center bg-white w-full mx-auto mt-10 mb-10">
				<div  class="mx-auto border-space-cadet flex border-2 rounded-lg md:rounded-xl w-full h-12 box-border pl-5 mb-3 overflow-hidden">
					<input class="w-11/12 h-5/6 my-auto outline-none text-xl" type="text" v-model="filterByText" placeholder="Rechercher..." />
					<FontAwesomeIcon class="mr-3 sm:mr-0 h-1/2 my-auto" icon="fa-magnifying-glass"/>
				</div>
	
				<div class="flex flex-row justify-center space-x-5">
					<div class="flex flex-col space-y-3 w-1/2">
						<div class="flex h-12 border-space-cadet border-2 rounded-lg md:rounded-xl justify-between">
							<label class="w-3/5 p-2 text-md lg:text-xl" for="typeFormation">Formation</label>
							<select
						...
						<div class="flex h-12 border-space-cadet border-2 rounded-lg md:rounded-xl justify-between">
							<label class="w-3/5 p-2 text-md lg:text-xl" for="Structure">Structure</label>
							<select class="w-1/3 h-2/3 mr-5 my-auto bg-white" name="Structure" id="Structure" v-model="inputStructure">
						...
						<div class="flex h-12 border-space-cadet border-2 rounded-lg md:rounded-xl justify-between">
							<label class="w-3/5 p-2 text-md lg:text-xl" for="access">Accès</label>
							<select class="w-1/3 h-2/3 mr-5 my-auto bg-white" name="access" id="access" v-model="inputAccess">
					...
					<div class="flex flex-col w-1/2 justify-center space-y-2 border-space-cadet border-2 rounded-lg md:rounded-xl">
						<h2 class="font-bold pl-2 text-center text-2xl">Durée</h2>
						<div class="flex justify-between sm:justify-center h-12 w-full">
							<label class="w-1/4 grid place-items-center text-md md:text-xl" for="minDuration">Min</label>
							<input
						...
						<div class="flex justify-center h-12">
							<label class="w-1/4 grid place-items-center text-md md:text-xl" for="maxDuration">Max</label>
							<input
								class="w-3/4 sm:w-2/5 h-2/3 mr-5 p-5 my-auto border-space-cadet border-2 outline-none rounded-md md:rounded-xl"
								type="number"
								name="maxDuration"
			...
			<div class="w-full h-96 mx-auto rounded-xl border-space-cadet border-2 overflow-hidden mb-10">
				<MapFormations v-if="maploaded" :formations="filteredFormations" />
				<div v-else class="text-center bg-gradient-to-r from-space-cadet via-white to-space-cadet h-full grid place-items-center">
					<button
