<template>
    <div class="w-1/2 m-auto mt-5 flex flex-col gap-4 font-inter">
        <div class="border-space-cadet border-2 rounded-lg"
            :class="{'border-white border-2 rounded-bl-lg rounded-br-lg': isHidden,}">
            <div class="h-16 flex items-center justify-between"
                :class="{'bg-space-cadet text-white border-b-2 rounded-tl-lg rounded-tr-lg': isHidden}">
                <div class="cursor-pointer border-r-2 h-16 w-16 flex justify-center items-center border-space-cadet"
                    @click="select()"
                    :class="{'border-white': isHidden, 'bg-space-cadet text-white': isSelected && !isHidden}">
                    <FontAwesomeIcon icon="fa-circle-plus" class="h-5" v-if="!isSelected" />
                    <FontAwesomeIcon icon="fa-circle-minus" class="h-5" v-if="isSelected" />
                </div>

                <h1 class="pl-5 pr-5">{{ disp.formationName }}</h1>

                <div class="cursor-pointer border-l-2 h-16 w-16 flex justify-center items-center border-space-cadet"
                    :class="{'border-white': isHidden}" @click="toggle()">
                    <FontAwesomeIcon icon="fa-caret-down" v-if="!isHidden" class="h-5" />
                    <FontAwesomeIcon icon="fa-caret-up" v-if="isHidden" class="h-5" />
                </div>

            </div>
            <div v-if="isHidden" class="bg-space-cadet p-5 m-auto flex flex-col gap-2 text-white"
                :class="{'border-white border-b-2 rounded-bl-lg rounded-br-lg':isHidden}">
